...
# initial condition
X0 = rep(0, 8)  # initializing the initial population vector
...
data_seir = list(
	...
	#p_A = c(-0.0011, 0.005),  #from mordecai 2019 (for Anopheles gambiae!)
	#p_B = c(0.049, 0.005),  
	#p_C = c(0.609, 0.05), 
	#p_mu_v_q = c(1, 0.000005),
	p_q_birth = c((2), 10),  #from mordecai 2019 (for Anopheles gambiae!)
	p_q_mu_v = c(0.5,10),
	p_q_kappa = c(2, 10),  #from mordecai, 2013
	
	# initial condition
	y0_vars = y0_vars
)

MalariaModel_Full <- stan_model(paste(path, "Adamaoua-feb10.stan", sep="") )
...
# fit_seir <- sampling(MalariaModel_Full, data = data_seir, iter = 100,  chains = 4)
 ...
 fit_seir <- sampling(MalariaModel_Full, data = data_seir, iter = 1000,  init = function() list(q_mu_v=0.2, i0 = 5000, q = 1, Iv0 = 500, w_h = 0.5, delta = 0.1, q_birth = 1, q_kappa =1), chains = 4)
...
# fit_seir <- sampling(MalariaModel_Full, data = data_seir, iter = 100, chains = 1)
 ...
 Post  <- data.frame(i0 = Extract$i0, q_birth = Extract$q_birth, q_b = Extract$q,
                     Iv_0 = Extract$Iv0,  delta = Extract$delta,
                     w_h = Extract$w_h, phi = Extract$phi, q_kappa = Extract$q_kappa)
 write.table(Post, file="Par_PostDist_Adamaoua.csv")
 
 # parameters for the prior for phi 
 Post <- read.table(file= paste(path, "Par_PostDist_Adamaoua.csv", sep="") )
 
 
 
 
 # checking inference
 pars = c( "i0", "q",  "q_birth", "Iv0", 'delta', 'w_h', 'phi', "q_kappa", "q_mu_v") # specifying parameters of interest
